...
export default function DashboardPage() {
  ...
  // 센서가 복귀(OUT_OF_RANGE 해제)하면 dismissedIds에서 자동 제거 → 재이탈 시 카드 재등장
  useEffect(() => {
    const currentIds = new Set(deviatedSlots.map(s => s.id))
    setDismissedDeviationIds(prev => {
  ...
  return (
    <div className="dashboard" ref={containerRef}>
      ...
      {/* 이탈 패널 — 닫지 않은 OUT_OF_RANGE 슬롯이 1개 이상인 경우 표시 */}
      {/* TODO: 팝업 화면 추가 필요 — DeviationCard 클릭 시 상세 이탈 이력/차트 팝업 */}
      {visibleDeviatedSlots.length > 0 && (
        <DeviationPanel
          slots={visibleDeviatedSlots}
          deviationStats={deviationStats}
          expandTrigger={deviationExpandKey}
          onDismiss={(id) =>
            setDismissedDeviationIds(prev => new Set([...prev, id]))
          }
        />
      )}
